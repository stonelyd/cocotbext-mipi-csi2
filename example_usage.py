...
def demonstrate_basic_features():
    ...
    print(f"D-PHY: {dphy_config.lane_count} lanes @ {dphy_config.bit_rate_mbps} Mbps")
    ...
    print(f"C-PHY: {cphy_config.trio_count} trios @ {cphy_config.bit_rate_mbps} Mbps")
    ...
    print(f"RGB888 Packet: VC={rgb_packet.virtual_channel}, Size={len(rgb_packet.payload)} bytes")
    ...
    print(f"ECC Calculation: DI=0x{data_id:02x}, WC={word_count} -> ECC=0x{ecc:02x}")
    ...
    print(f"Checksum Validation: {len(test_payload)} bytes -> Valid={is_valid}")
    ...
    data_types = [
        ...
        DataType.YUV422_8BIT, DataType.YUV420_8BIT,
        DataType.FRAME_START, DataType.FRAME_END, DataType.LINE_START, DataType.LINE_END
    ]
    ...
    configs = [
        ("D-PHY 1-lane 1Gbps", Csi2Config(PhyType.DPHY, lane_count=1, bit_rate_mbps=1000)),
        ("D-PHY 4-lane 2.5Gbps", Csi2Config(PhyType.DPHY, lane_count=4, bit_rate_mbps=2500)),
        ("C-PHY 1-trio 2Gbps", Csi2Config(PhyType.CPHY, trio_count=1, bit_rate_mbps=2000)),
        ("C-PHY 3-trio 3Gbps", Csi2Config(PhyType.CPHY, trio_count=3, bit_rate_mbps=3000)),
    ]
...
def demonstrate_advanced_features():
    ...
    print(f"• Example: {error_config.error_injection_rate*100}% error injection rate")
