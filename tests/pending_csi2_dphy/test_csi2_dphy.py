...
from cocotbext.mipi_csi2.config import Csi2PhyConfig
...
async def test_dphy_lane_distribution(dut):
    ...
    # Create separate TX and RX PHY models
    tx_phy_model = DPhyTxModel(bus, config)
    rx_phy_model = DPhyRxModel(bus, config)

    tx_model = Csi2TxModel(bus, config, phy_model=tx_phy_model)
    rx_model = Csi2RxModel(bus, config, phy_model=rx_phy_model)

    # Set up RX callbacks
    rx_phy_model.set_rx_callbacks(
        on_packet_start=rx_model._on_phy_packet_start,
        on_packet_end=rx_model._on_phy_packet_end,
        on_data_received=rx_model._on_phy_data_received
    )
